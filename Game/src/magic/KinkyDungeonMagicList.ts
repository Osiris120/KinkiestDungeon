...
let KinkyDungeonSpellList: Record<string, spell[]> = { // List of spells you can unlock in the 3 books. When you plan to use a mystic seal, you get 3 spells to choose from.
	...
	"Conjure": [
		...
		{goToPage: 5, name: "FloatingWeapon", increasingCost: true, tags: ["magic"], autoLearn: ["RecoverObject"],
			hideUnlearnable: true, school: "Conjure", manacost: 0,
			mixedPassive: true,
			spellPointCost: 1, components: [], level:1, type:"passive", onhit:"",
...
let KinkyDungeonSpellListEnemies: spell[] = [
	...
	{enemySpell: true, name: "OneBarMissile",
		bindType: "Metal", color: "#ffffff", sfx: "MagicSlash", landsfx: "MagicSlash",
		manacost: 6, components: ["Arms"], specialCD: 25,
		noTerrainHit: true,
		pierceEnemies: true,
		faction: "Warden",
		minRange: 0,
		...
		events: [{type: "RubberMissileHoming", trigger: "bulletAfterTick", power: 0.4,
			dist: 15, count: 0.2, limit: 0},],
		bind: 8,
...
let KDCastConditions: Record<string, (enemy: entity, target: entity, spell?: spell) => boolean> = {
	...
	"EnemyEnchantRope": (_enemy, target) => {
		if (target.player && KinkyDungeonPlayerTags.get("RopeSnake")) {
			return MiniGameKinkyDungeonLevel > 1;
		}
		else if (target?.specialBoundLevel?.Rope > 0) return MiniGameKinkyDungeonLevel > 1;
		return false;
	},
